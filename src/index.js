...
export default class MioxKoaVue2xServerRender {
    ...
    RUN_IN_PRODUCTION() {
        const PATH_BUILD_PREFIX = path.resolve(process.cwd(), this.options.buildPrefix);
        const PATH_SSR_BUNDLE = path.resolve(PATH_BUILD_PREFIX, 'vue-ssr-bundle.json');
        const PATH_SSR_TEMPLATE = path.resolve(PATH_BUILD_PREFIX, 'index.html');
    ...
    SETUP_DEV_SERVER(cb) {
        ...
        clientCompiler.plugin('done', () => {
            ...
            const filePath = path.join(this.clientConfig.output.path, 'index.html');
        ...
        serverCompiler.watch({}, (err, stats) => {
            ...
            const bundlePath = path.join(this.serverConfig.output.path, 'vue-ssr-bundle.json');
